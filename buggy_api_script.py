...
import json
import os  # missing import for os to check/create directories

API_URL = "https://catfact.ninja/fact"  # fact instead of facts

def get_cat_fact():
    """Fetch a single cat fact string from the API and return it."""
    # NOTE: There may be a few bugs below. Read errors carefully!
    resp = requests.get(API_URL, timeout=5)  #wrong data type for timeout (should be int or float, not string)
...
def save_fact_to_file(text):
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(text)
    print("Saved to cat_fact.txt")  #missing final t
